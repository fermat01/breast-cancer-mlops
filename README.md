...
│   ├── validate.py       # Validate dataset
│   ├── split.py          # Train/test split
│   ├── preprocess.py     # Feature preprocessing
│   ├── model.py          # Model creation + training
...
1. **Data Pipeline** — loading, validation, preprocessing, train/test split.
2. **Model Training** — train the classifier, evaluate it, save the model.
3. **MLflow** — track every experiment and register the trained model.
4. **FastAPI** — expose the trained model through a clean API using Pydantic.
5. **Testing** — verify preprocessing, training, and API endpoints.
6. **Prometheus** — monitor requests and prediction latency.
7. **Docker** — containerize the complete application.
8. **GitHub Actions** — automate testing and image builds.
9. **AWS ECS (Fargate)** — deploy the application.
10. **Production hardening** — configuration, logging, documentation, and performance improvements.
...
_Happy MLOps development and breast cancer prediction!_
