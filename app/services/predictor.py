...
def predict(features: list[float]) -> dict[str, Any]:
    ...
    try:
        ...
        logger.debug(
            "Using model metadata: name=%s alias=%s version=%s run_id=%s",
            model_metadata.name,
            model_metadata.alias,
        ...
        PREDICTIONS_BY_CLASS.labels(class_label=prediction_label).inc()
        ...
        return {
            ...
            "model_alias": model_metadata.alias,
        ...
        PREDICTION_LATENCY_SECONDS.observe(elapsed_time)

        logger.debug(
            "Prediction request completed in %.4f seconds",
            elapsed_time,
        )
